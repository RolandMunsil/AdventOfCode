...
cur_paths = [[0, grid.pos_of('S'), Pos.new(1, 0), Set[grid.pos_of('S')]]]
...
until cur_paths.empty?
  ...
  next if !best_paths.empty? && best_paths_cost < cur_cost
  ...
  if grid[start_pos] == 'E'
    if best_paths.empty?
      best_paths.append cur_best_path
      best_paths_cost = cur_best_path[0]
    else
      best_paths.append cur_best_path
    end
  end
  ...
  valid_options.each do |next_pos|
    ...
    next_path = if cur_dir == next_dir
                  [cur_cost + 1, next_pos, next_dir, visited_tiles | [next_pos]]
                elsif cur_dir == -next_dir
                  [cur_cost + 2001, next_pos, next_dir, visited_tiles | [next_pos]]
                else
                  [cur_cost + 1001, next_pos, next_dir, visited_tiles | [next_pos]]
                end
...
puts best_paths.map { |path| path[3] }.inject(:+).length