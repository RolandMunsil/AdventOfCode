...
let endPos: Pos = (grid[0].high, grid.high)
...
while priQueue.len > 0:
    ...
    var edges: seq[seq[Pos]]

    for n in 4..10:
        let vHigh = node.pos + (node.axis * n)

        if vHigh.isValid:
            edges.add((1..n).mapIt(node.pos + (node.axis * it)))

        let vLow = node.pos - (node.axis * n)
